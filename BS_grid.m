...
low = 3400;       % lower limit of window
high = 3600;       % upper limit of window
movestep = 200;
sampleN=length(AGE);

for j = 1:1:(low/movestep)   % remove outliers for each 200 Myr-bin
...
Group_data = grpstats(Clip_ds,'Ident',{'mean'});
...
for j = 1:1:((low+high)/2/movestep+1)   % remove outliers for each window
    ...
    if n(j)>=4       % less than 10 samples will not be calculated.
...
csvwrite('BS_gird_P2O5_300_2.csv',result);
